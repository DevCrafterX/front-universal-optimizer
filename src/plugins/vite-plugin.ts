...
export default function ViteOptPlugin(config: OptimizeConfig): Plugin {
  ...
  return {
    ...
    config(viteConfig) {
      const chunkSplitConfig =
        typeof config.chunkSplit === 'object' ? config.chunkSplit : { enable: config.chunkSplit };
    ...
    transform(code, id) {
      ...
      if (!supportedExtensions.test(id)) {
        return null;
      }
      ...
      if (config.env === 'production' && config.clearConsole) {
        ...
        if (strategy === 'regex') {
          ...
          console.warn(
            '[front-universal-optimizer] clearConsole 启用，请确保项目已配置 @babel/plugin-transform-remove-console',
          );
          return null;
        }
      }
    ...
    transformIndexHtml(html) {
      ...
      if (config.domPrefetch) {
        res = res.replace(
          '<head>',
          '<head><!-- dns-prefetch 建议：添加 <link rel="dns-prefetch" href="//your-api-domain.com"> -->',
        );
      }
      return res;
    },
